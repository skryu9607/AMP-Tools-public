...
//     amp::Path2D path;
//     path.waypoints.push_back(problem.q_init);
//     Graph rrtTree; // Use the Graph structure to store nodes and edges

//     CollisionChecker checker;
//     size_t maxIterations = 1000; // Maximum number of iterations to build the tree
//     double stepSize = 0.5; // Step size for extending the tree

//     // Node IDs for start and goal
//     int startNodeId = 0;
//     int goalNodeId = 1;

//     // Add initial configuration as the root of the RRT
//     Eigen::Vector2d startPosition(problem.q_init.x(), problem.q_init.y());
//     rrtTree.addNode(startNodeId, startPosition);

//     // Random number generation setup
//     std::random_device 42;
//     std::mt19937 gen(42);
//     std::uniform_real_distribution<> disX(problem.x_min, problem.x_max);
//     std::uniform_real_distribution<> disY(problem.y_min, problem.y_max);
...
//         Eigen::Vector2d randomPoint(disX(gen), disY(gen));

//         // Find the nearest node in the tree to the sampled point
//         int nearestNodeId = -1;
//         double minDistance = std::numeric_limits<double>::infinity();

//         for (const auto& [id, node] : rrtTree.getNodes()) {
//             double distance = (node.position - randomPoint).norm();
//             if (distance < minDistance) {
//                 minDistance = distance;
//                 nearestNodeId = id;
//             }
...
//         // Compute the direction and move a step size towards the random point
//         Eigen::Vector2d nearestNodePosition = rrtTree.getNode(nearestNodeId).position;
//         Eigen::Vector2d direction = (randomPoint - nearestNodePosition).normalized();
//         Eigen::Vector2d newPoint = nearestNodePosition + stepSize * direction;

//         // Check if the new point is in collision
//         if (!checker.checkPointCollision(newPoint, problem.obstacles)) {
//             int newNodeId = rrtTree.getNodes().size();
//             rrtTree.addNode(newNodeId, newPoint);
//             rrtTree.addEdge(nearestNodeId, newNodeId);

//             // Check if we have reached the goal
//             if ((newPoint - Eigen::Vector2d(problem.q_goal.x(), problem.q_goal.y())).norm() < stepSize) {
//                 rrtTree.addNode(goalNodeId, Eigen::Vector2d(problem.q_goal.x(), problem.q_goal.y()));
//                 rrtTree.addEdge(newNodeId, goalNodeId);
//                 goalReached = true;
//                 break;
//             }
//         }
...
//             for (const auto& [id1, id2] : rrtTree.getEdges()) {
//                 if (id2 == current) {
//                     current = id1;
//                     break;
//                 }
...
//         // Convert the path nodes to waypoints
//         for (const auto& nodeId : pathNodes) {
//             path.waypoints.push_back(rrtTree.getNode(nodeId).position);
//         }
//     } else {
//         std::cout << "RRT could not find a path to the goal." << std::endl;
//     }
//     path.waypoints.push_back(problem.q_goal);
//     return path;
// }
